...
export const fetchAlbums = createAsyncThunk(
    ...
    async ({ page = 1, order = 'asc', limit = 10 }, { getState }) => {
        try {
            ...
            const response = await axios.get(`${URL_SERV}/albums?_page=${page}&_limit=${limit}&_order=${order}&_sort=id`);
            const prevState = getState().albums;
...
export const fetchAlbumById = createAsyncThunk(
    ...
    async (id) => {
        try {
            const response = await axios.get(`${URL_SERV}/albums/${id}`);
